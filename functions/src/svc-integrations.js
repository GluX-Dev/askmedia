...
class PaystackHelper {
  ...
  async verifyTransaction(reference) {
    debugLog(`[Paystack] Verifying transaction: ${reference}`);
    try {
      ...
      if (result?.status && result?.data?.status === "success") {
        debugLog(`[Paystack] Verified ref=${reference} amount=${(result.data.amount / 100).toFixed(2)} ${result.data.currency || "GHS"}`);
      }
      return result;
    } catch (error) {
...
class HubnetHelper {
  constructor(apiKey, baseUrl) {
    ...
    const masked = this.apiKey.length > 8
      ...
      : "[short]";
    console.log(`[Hubnet] Client ready auth=${this.authMode} base=${this.baseUrl} key=${masked}`);
    debugLog("[Hubnet] Rate limit is approximately 5 requests/minute.");
  }
  ...
  async request(endpoint, options = {}) {
    ...
    debugLog(`[Hubnet] ${method} ${url} (${timeoutMs}ms)`);
    if (options.body) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          console.error("[Hubnet] Rate limited (429).");
        }
      ...
      if (payload && (this.isDocErrorPayload(payload) || (this.isFailureStatus(payload.status) && !this.isDocSuccessPayload(payload)))) {
        ...
        console.error(`[Hubnet] API failure: ${message} | status=${payload.status} | msgCode=${payload.message}`);
      ...
      if (payload && !this.isDocSuccessPayload(payload) && !sanitizeStringId(payload?.transaction_id) && !sanitizeStringId(payload?.payment_id)) {
        ...
        if (this.isFailureStatus(payload.status) || numericErr || rc >= 400) {
          ...
          console.error(`[Hubnet] API failure: ${message}`);
      ...
      const summary = payload
        ? `status=${payload.status} msgCode=${payload.message} reason=${payload.reason || "â€”"} tx=${payload.transaction_id || "â€”"} pay=${payload.payment_id || "â€”"}`
        : "empty";
      debugLog(`[Hubnet] OK (${elapsed}ms) | ${summary}`);
      return payload;
    } catch (err) {
      ...
      if (!err.statusCode) {
        console.error(`[Hubnet] Network error: ${err.message}`);
      }
      throw err;
    } finally {
      clearTimeout(timer);
...
class FulfillmentHelper {
  ...
  async createOrder(orderData) {
    debugLog("[FallbackFulfill] create order");
    debugLog(`[FallbackFulfill] ref=${orderData.externalReference}`);
    ...
    debugLog("[FallbackFulfill] created");
    return response.data;
  }
